...
/**
 * 根据灰度上下文决定路由规则，从而返回指定的服务机器
 */
public class RibbonLoadBalancerClientInterceptor implements Interceptor {
    ...
    public Object intercept(Object obj, Object[] allArguments, Callable<?> zuper, Method method) throws Throwable {
        if(allArguments[0] != null && allArguments[0] instanceof ILoadBalancer) {
            if (!EurekaApplicationsHolder
                    .checkApplicationIsGray(((DynamicServerListLoadBalancer) allArguments[0]).getName().toUpperCase()) ) {
                return zuper.call();
            }
            Object ret = null;
            try {
                Server server = chooseServer(allArguments[0]);
                if (server != null) {
    ...
    private Server chooseServer(Object arg) throws Exception {
        List<Server> serverList = new ArrayList<>();

        // 获取所有的服务列表
        try {
            ILoadBalancer iLoadBalancer = (DynamicServerListLoadBalancer)arg;
            ...
            serverList = predicate.getEligibleServers(iLoadBalancer.getAllServers(), "default");
