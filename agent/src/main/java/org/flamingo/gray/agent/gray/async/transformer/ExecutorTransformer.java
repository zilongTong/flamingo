                ...
                for (CtMethod method : classInfo.getCtClass().getDeclaredMethods()) {
                    ...
                    for (int i = 0; i < parameterTypes.length; i++) {
                        ...
                        if (RUNNABLE_CLASS_NAME.equals(paramTypeName)) {
                            code += String.format("$%d = org.flamingo.gray.agent.gray.async.RunnableWrapper.wrapRunnable($%d);", i + pos, i + pos);
                        } else if (CALLABLE_CLASS_NAME.equals(paramTypeName)) {
                            code += String.format("$%d = org.flamingo.gray.agent.gray.async.RunnableWrapper.wrapCallable($%d);", i + pos, i + pos);
                        }
                    }
